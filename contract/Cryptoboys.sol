...
contract Homer is ERC721Metadata {
  ...
  modifier periodStarted() {
    require(block.timestamp >= startTime, "Homer: Period not started");
    _;
  }
  ...
  constructor(
    ...
    address admin_,
    address admin2_
  ) ERC721Metadata("Homer", "AVAPEPE", baseURI_) {
    _admin = admin_;
    _admin2 = admin2_;
  }

  function setStartTime(uint256 _startTime) external onlyOwner {
    require(_startTime > 0, "Homer: invalid _startTime");
    require(_startTime > block.timestamp, "Homer: old start time");
  ...
  function setGiveawayMax(uint256 _giveawayMax) external onlyOwner {
    require(
      ...
      "Homer: invalid max value"
    );
    require(giveawayMax != _giveawayMax, "Homer: already set");
    giveawayMax = _giveawayMax;
  }

  function randomGiveaway(address to) external onlyOwner {
    require(to != address(0), "Homer: zero address");
    require(
      _giveawayCounter.current() < giveawayMax,
      "Homer: overflow giveaways"
    );
  ...
  function purchase(uint256 numberOfNfts) external payable periodStarted {
    require(pendingCount > 0, "Homer: All minted");
    require(numberOfNfts > 0, "Homer: numberOfNfts cannot be 0");
    require(
      numberOfNfts <= 20,
      "Homer: You may not buy more than 20 NFTs at once"
    );
    require(
      totalSupply().add(numberOfNfts) <= MAX_NFT_SUPPLY,
      "Homer: sale already ended"
    );
    require(
      MINT_PRICE.mul(numberOfNfts) == msg.value,
      "Homer: invalid ether value"
    );
  ...
  function claimReward(uint256 tokenId) public {
    require(
      ...
      "Homer: Only owner or approved can claim rewards"
    );
    require(
      _giveaways[tokenId] == address(0),
      "Homer: can't claim for giveaways"
    );
  ...
  function _randomMint(address _to) internal returns (uint256) {
    require(totalSupply() < MAX_NFT_SUPPLY, "Homer: max supply reached");
